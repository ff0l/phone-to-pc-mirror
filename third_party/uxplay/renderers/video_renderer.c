...
static guintptr g_overlay_hwnd = 0;

void video_renderer_set_window_handle(guintptr handle) {
    g_overlay_hwnd = handle;
}

static void apply_overlay_handle(GstElement *pipeline) {
    GstElement *sink;
    if (!g_overlay_hwnd || !pipeline) {
        return;
    }
    ...
    gst_video_overlay_set_window_handle(GST_VIDEO_OVERLAY(sink), g_overlay_hwnd);
...
void video_renderer_init(logger_t *render_logger, const char *server_name, videoflip_t videoflip[2], const char *parser, const char * rtp_pipeline,
    ...
    for (int i = 0; i < n_renderers; i++) {
        ...
        apply_overlay_handle(renderer_type[i]->pipeline);
        GstState state;
...
int video_renderer_choose_codec (bool video_is_jpeg, bool video_is_h265) {
    ...
    apply_overlay_handle(renderer->pipeline);
    gst_element_set_state (renderer->pipeline, GST_STATE_PLAYING);
    GstState old_state, new_state;
