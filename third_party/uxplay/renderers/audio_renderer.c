...
void audio_renderer_init(logger_t *render_logger, const char* audiosink, const bool* audio_sync, const bool* video_sync, const char *artp_pipeline) {
    ...
    for (int i = 0; i < NFORMATS ; i++) {
        ...
        g_string_append(launch, "queue max-size-time=40000000 max-size-buffers=4 ! ");
        switch (i) {
        case 0:    /* AAC-ELD */
        case 2:    /* AAC-LC */
...
void audio_renderer_stop() {
    if (renderer) {
        ...
        gst_element_get_state(renderer->pipeline, NULL, NULL, 200 * GST_MSECOND);
        renderer = NULL;
    }
}
...
void audio_renderer_flush() {
    ...
    pad = gst_element_get_static_pad(renderer->appsrc, "src");
    if (pad) {
