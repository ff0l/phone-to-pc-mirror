...
#ifdef UXPLAY_AS_LIBRARY
#include "uxplay_api.h"
static void emit_host_event(int event, const char *text);
#endif
...
#ifdef UXPLAY_AS_LIBRARY
    dnssd_set_airplay_features(dnssd, 8, 1);
#endif
...
#ifdef UXPLAY_AS_LIBRARY
    emit_host_event(UXPLAY_EVENT_CLIENT, NULL);
#endif
}
...
#ifdef UXPLAY_AS_LIBRARY
        emit_host_event(UXPLAY_EVENT_IDLE, NULL);
#endif
    }
}
...
extern "C" void video_report_size(void *cls, float *width_source, float *height_source, float *width, float *height) {
    if (use_video) {
        ...
        {
            char sizebuf[64];
            int sw = (int) (*width_source > 0.0f ? *width_source : *width);
            int sh = (int) (*height_source > 0.0f ? *height_source : *height);
            if (sw > 0 && sh > 0) {
                snprintf(sizebuf, sizeof(sizebuf), "%dx%d", sw, sh);
                emit_host_event(UXPLAY_EVENT_SIZE, sizebuf);
            }
        }
#endif
    }
...
#ifdef UXPLAY_AS_LIBRARY
static uxplay_event_cb g_host_event_cb = NULL;
static void *g_host_event_cls = NULL;

void uxplay_set_event_callback(uxplay_event_cb cb, void *cls) {
    g_host_event_cb = cb;
    g_host_event_cls = cls;
}

static void emit_host_event(int event, const char *text) {
    if (g_host_event_cb) {
        g_host_event_cb(event, text, g_host_event_cls);
    }
}
...
#ifdef UXPLAY_AS_LIBRARY
    emit_host_event(UXPLAY_EVENT_READY, server_name.c_str());
#endif
    reconnect:
    compression_type = 0;
